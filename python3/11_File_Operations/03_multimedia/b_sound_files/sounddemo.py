...
def ease(t, exp):
    t -= np.floor(t)
    if t < 0.5:
        return 0.5 * np.power(2 * t, exp)
    else:
        return 1 - 0.5 * np.power(2 * (1 - t), exp)
...
if __name__ == "__main__":
    ...
    data = mix(sinewave(10, 300), sawtooth(10, 1 / 100), wt=lambda t: np.sin(np.pi * t))
    data = mix(data, fractal(10, 99), wt=lambda t: 1 - 0.8 * blip(t, 4, 9, 1))
