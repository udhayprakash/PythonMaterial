...
class SmartBufferHandler(logging.handlers.MemoryHandler):
    def __init__(self, num_buffered, *args, **kwargs):
        # +2 one for current, one for prepop
        kwargs["capacity"] = num_buffered + 2
...
handler = SmartBufferHandler(
    num_buffered=2, target=logging.StreamHandler(), flushLevel=logging.ERROR
)
...
logger.debug("Hello3")
logger.debug("Hello4")
# As error will flush the buffered logs, the two last debugs will be logged
logger.error("Hello5")
